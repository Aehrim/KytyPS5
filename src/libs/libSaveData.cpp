...
// Saves created by this fork before upstream persisted allocations (2026-09-17 .. 09-23) keep
// their block count in a text file "sce_kyty_blocks" next to the data. Convert it once so
// those saves stay loadable instead of reporting SAVE_DATA_ERROR_BROKEN.
static void migrate_legacy_save_blocks(const std::filesystem::path& directory) {
	std::error_code error;
	const auto      current = directory / "sce_sys" / "blocks.bin";
	const auto      legacy  = directory / "sce_kyty_blocks";
	...
	input.close();
	blocks = std::clamp(blocks, SAVE_DATA_BLOCKS_MIN, SAVE_DATA_BLOCKS_MAX);
	std::filesystem::create_directories(directory / "sce_sys", error);
	if (!error && write_save_file(current, &blocks, sizeof(blocks)) == OK) {
		std::filesystem::remove(legacy, error);
	}
}

static int read_save_blocks(const std::filesystem::path& directory, uint64_t* blocks) {
	migrate_legacy_save_blocks(directory);
...
int KYTY_SYSV_ABI SaveDataGetMountInfo(const SaveDataMountPoint* mount_point,
	...
	uint64_t  blocks = 0;
	const int status =
	    read_save_blocks(LibKernel::FileSystem::GetRealFilename(mount_point->data), &blocks);
	if (status != OK) {
		return status;
	}
	*info             = {};
