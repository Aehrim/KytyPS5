...
TextureCache::OverlapResult TextureCache::ResolveOverlap(const ImageInfo& requested,
	...
	if (requested.data.address == cached.info.data.address &&
		...
		// A larger, incompatible alias of cached memory (Demon's Souls binds a 3345-layer view
		// over a 64-layer array) cannot be merged; bind a null image rather than abort the draw.
		static std::atomic<uint32_t> overlap_log_count {0};
		if (overlap_log_count.fetch_add(1) < 16u) {
			LOGF(
			    "TextureCache: unresolvable equal-address image overlap, binding null: "
			    "address=0x%016" PRIx64 " requested=%ux%u cached=%ux%u requested_size=0x%016" PRIx64
			    " cached_size=0x%016" PRIx64 " type=%u/%u tile=%u/%u\n",
