...
bool BufferCache::DownloadBufferMemory(Buffer& buffer, uint64_t vaddr, uint64_t size) {
	// A single staging reservation is limited by the download ring, so large GPU-modified
	// ranges are split and downloaded in batches.
	constexpr uint64_t          batch_limit = 16 * MiB;
	...
	m_memory_tracker.ForEachDownloadRange<false>(
	    vaddr, size, [&](uint64_t address, uint64_t bytes) noexcept {
		    m_memory_tracker.ValidateGpuDirtyPages(m_gpu_modified_ranges, address, bytes,
		                                           "buffer download");
		    m_gpu_modified_ranges.ForEachInRange(address, bytes, [&](uint64_t start, uint64_t end) {
			    for (uint64_t piece = start; piece < end; piece += batch_limit) {
				    ranges.emplace_back(piece - buffer_address, 0,
				                        std::min(batch_limit, end - piece));
			    }
		    });
		    m_gpu_modified_ranges.Subtract(address, bytes);
	    });
	...
	for (size_t first = 0; first < ranges.size();) {
		std::vector<vk::BufferCopy> copies;
		uint64_t                    total_size = 0;
		while (first < ranges.size()) {
			// Keep packed ranges on separate cache lines, as in shadPS4.
			const auto padded = Common::AlignUp(ranges[first].size, 64);
			if (!copies.empty() && total_size + padded > batch_limit) {
				break;
			}
		...
		if (mapped == nullptr) {
			EXIT("BufferCache: download of %" PRIu64 " bytes does not fit the staging buffer\n",
			     total_size);
		}
		...
		// Mapping can wait on the ring and submit the previous batch, so the command buffer is
		// looked up per batch.
