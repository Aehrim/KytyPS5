...
public:
	...
	// Changes whenever an image is created or deleted.
	[[nodiscard]] uint64_t MutationEpoch() const noexcept {
		return m_mutation_epoch.load(std::memory_order_relaxed);
	}
...
private:
	...
	std::atomic<uint64_t>                             m_mutation_epoch {0};
