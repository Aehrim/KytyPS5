...
// Saves written by the fork before upstream persisted allocations keep the block count in a
// text file next to the data. They must stay loadable and move to sce_sys/blocks.bin once.
void TestLegacyBlockMigration() {
	Reset("LEGACY");
	const fs::path root = "_SaveData/LEGACY";
	const std::array<const char*, 3> dirs {"Options", "Player", "Tiny"};
	const std::array<const char*, 3> legacy {"48", "96", "20"};
	const std::array<uint64_t, 3>    expected {48, 96, 48};
	for (size_t i = 0; i < dirs.size(); i++) {
		CHECK(fs::create_directories(root / dirs[i]));
		std::ofstream(root / dirs[i] / "USR-DATA") << "saved payload";
		std::ofstream(root / dirs[i] / "sce_kyty_blocks") << legacy[i];
	}
	std::array<SceSaveDataDirName, 3> found {};
	std::array<SaveDataSearchInfo, 3> infos {};
	SaveDataDirNameSearchCond cond {};
	cond.user_id = 1;
	SaveDataDirNameSearchResult search {};
	search.dir_names     = found.data();
	search.dir_names_num = found.size();
	search.infos         = infos.data();
	CHECK(SaveDataDirNameSearch(&cond, &search) == OK && search.set_num == 3);
	for (size_t i = 0; i < dirs.size(); i++) {
		CHECK(std::string(found[i].data) == dirs[i]);
		CHECK(infos[i].blocks == expected[i] && infos[i].free_blocks == expected[i]);
		CHECK(fs::file_size(root / dirs[i] / "sce_sys" / "blocks.bin") == sizeof(uint64_t));
		CHECK(!fs::exists(root / dirs[i] / "sce_kyty_blocks"));
		CHECK(fs::exists(root / dirs[i] / "USR-DATA"));
	}
	// A second search reads the converted files.
	CHECK(SaveDataDirNameSearch(&cond, &search) == OK && search.set_num == 3);
	CHECK(infos[1].blocks == 96);
}
